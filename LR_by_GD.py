...
# 图像 1 ：展示 100 个随机数据点
plt.plot(x_data, y_data, 'b.', label="Original data")  # 红色星形的点
plt.title("Linear Regression using Gradient Descent")
plt.legend()
plt.show()

# 构建线性回归模型
W = tf.Variable(tf.random_uniform([1], -1.0, 1.0), dtype=tf.float32, name='weight')  # 初始化 Weight
b = tf.Variable(tf.zeros([1]), dtype=tf.float32, name='bias')                      # 初始化 Bias
